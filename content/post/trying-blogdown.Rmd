...
# quick summary of concatenated data structure
skimr::skim(O)
```
...
($\sigma = 2 \cdot \sqrt{\mu} = 5$), and an outcome ($Y$) based on a specified
...
We then project the observed data ($O$) onto a working MSM for the outcome,
using a logistic regression with terms for all of the baseline covariates and
the treatment. The `summary` of the resultant `glm` object is displayed above.
Notice that the estimated coefficients for the terms corresponding to the
...
p <- p + stat_smooth(method = "glm", method.args = list(family = "binomial"),
                     formula = y ~ x)
p <- p + nima::theme_nima() + xlab("A (treatment)") + ylab("Y (outcome)")
print(p)
```
...
rendered on my old [Jekyll](https://jekyllrb.com/) blog. Glad I never have to do
that again `r emo::ji("awesome")`
