---
author: "Nima Hejazi"
categories: [ "R", "data science", "statistics", "computing" ]
tags: [ "R", "data science", "statistics", "computing" ]
date: "2017-06-29"
description: "Quick look at using R with some Java 'under the hood'"
featured: ""
featuredalt: ""
featuredpath: ""
linktitle: ""
...
`r round(abs(time_norm[3] - time_comp[3]), 2)` seconds, (roughly) a factor of
`r round(time_norm[3] / time_comp[3], 0)`. Maybe `renjin` can help us out even
more?
...
```{r table-make, echo=FALSE, eval=FALSE}
...
```{r table-print, eval=FALSE}
print(table)
```

The gain in computational time here is awesome! Using `renjin` -- even just as a
wrapper -- improves the time cost (on the system side) by a large factor
relative to the naive implementation and by quite a bit still when compared to
the bytecode-compiled version of our function. This was just a simple example,
but we were able to save so much computational time just by naively calling
`renjin`...and it took just a few extra characters to call it as a wrapper...
