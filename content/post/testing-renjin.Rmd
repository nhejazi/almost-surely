...
```{r reg_times}
time_norm <- system.time(bigsum(1e8))
time_comp <- system.time(bigsumc(1e8))
```
...
```{r renjin}
time_renjin <- system.time(renjin(bigsum(1e8)))
```
...
naive implementation and by quite a bit still (a factor of
`r round(table[2, 2] / table[3, 2], 0)`) when compared to the bytecode-compiled
version of our function. Damn -- I'm at a loss for words. This was just a simple
example, but we were able to save so much computational time just by naively
calling `renjin`...and it took just a few extra characters to call it as a
wrapper...
