...
__This post is a working draft and is published only to share with coauthors.__
...
[`caret` R package](https://topepo.github.io/caret/index.html):
...
__Aside on H2O__:
Here, we rely on the [implementation of PCA by
H2O.ai](http://docs.h2o.ai/h2o/latest-stable/h2o-docs/data-science/pca.html);
this implementation takes advantage of the H2O data science platform and is
available for R via the [`h2o`
package](https://cran.r-project.org/web/packages/h2o/index.html). Since we'll be
using H2O, we need to set up an appropriate cluster on our local machine with

```{r h2o_setup, message=FALSE}
library(h2o)
h2o.init()
```

Other than our PCA learner, we've also instantiated a logistic regression model
(`fglm_learner` above) based on the fast implementation available through the
[`speedglm` R package](https://cran.r-project.org/package=speedglm), as well
as a random forest model based on the canonical implementation available in the
